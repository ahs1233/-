...
## 11. حالة التنفيذ — المرحلة A (شرائح عمودية)

نُفِّذت المرحلة A كأربع شرائح عمودية، كلٌّ Pull Request مستقل مُدمَج، مع الحفاظ على
**سلوك الصفحة الرئيسية مطابقاً بايتاً ببايت** (قاعدة التنفيذ #2) حتى نهايتها.

| الشريحة | المحتوى | إثبات الصحّة |
|---|---|---|
| **PR1** | عقد `DiscoveryItem` + المُصنِّف المحايد `rankItems` + سجلّ المُسقِطات (تسجيل ذاتي) + مُسقِطا `offer`/`commerce_entity` | اختبار تغطية السجلّ (كل نوع مُفعَّل له Projector + WeightProfile + Capabilities + إشارات [0..1]) |
| **PR2** | منفذ `CandidateSource` (ثابت #2) + منفذ `TrustProvider` (ثابت #10) + تركيب ثقة مرجعي يطابق V1 | اختبارات عقد: الثقة تطابق `bayesian/5`؛ مصدر وهميّ يغذّي السجلّ بلا `switch(type)` |
| **PR3** | قسم «اليوم» يمرّ عبر المكدّس كاملاً (مصدر → مُسقِط → مُصنِّف + ثقة) | تكافؤ وحدة (`== rankMixed` بالضبط) + تكامل على Postgres + **دِيف فارغ** لمخرجات الصفحة الحقيقية (قديم/جديد) |
| **PR4** | حارس المعمارية `scripts/check-architecture.mjs` (يفرض الحواف الممنوعة آلياً في CI) | اختبار ذاتي مضمَّن + رصد مؤكَّد لمخالفة مزروعة حقيقية (`commerce → discovery`) |

**الدين التقني المتبقّي لنهاية المرحلة A** (يُسدَّد في المرحلة B عند إعادة صياغة الأسطح):
- بطاقات «اليوم» تُبنى من `toCard` القديم (لا `DiscoveryItem.card`) للحفاظ على سبب `TRUSTED_STORE`.
- الأقسام: الترند/جديد/الأعلى تقييماً/الأكثر مبيعاً/متاجر جديدة ما زالت على المحرّك القديم؛ `best_selling` تحديداً يفرز بـ `soldCount` (ليس إشارة مطبّعة) فيلزمه تعريف سطح خاص في المرحلة B.
- تركيب الثقة المرجعي يعيد صياغة صيغة `score.ts`؛ يُوحَّد عند إزالة المحرّك القديم.

**معيار نجاح المرحلة A (مُحقَّق):** إضافة نوع جديد (Marketplace/Service) = `Projector + WeightProfile + Card + Capabilities` فقط — دون لمس المُصنِّف أو السجلّ أو أيّ منطق مركزي. الأنواع تُسجِّل نفسها، والأسطح تقرأ `capabilities`، والحواف الممنوعة يفرضها الحارس آلياً.
