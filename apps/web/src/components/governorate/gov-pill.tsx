 ...
 * النافذة تُصيَّر عبر Portal إلى body: الترويسة تستخدم backdrop-filter وهو ينشئ
 * «كتلة احتواء» تحبس أيّ عنصرٍ fixed داخلها (فيظهر كشريطٍ علويّ). الـ Portal يهرب منها.
 */
export function GovPill({ current }: { current: { id: string; name: string } | null }) {
  ...
  return (
    <>
      ...
      {open && mounted &&
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 sm:items-center" onClick={() => current && setOpen(false)}>
            <div className="bg-card max-h-[80vh] w-full max-w-md overflow-hidden rounded-t-3xl border border-line text-neutral-100 shadow-2xl sm:rounded-3xl" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between border-b border-line p-4">
                <h2 className="flex items-center gap-2 font-bold">
                  <MapPin className="h-5 w-5 text-gold-400" /> اختر محافظتك
                </h2>
                {current && (
                  <button onClick={() => setOpen(false)} className="text-neutral-400 hover:text-neutral-200" aria-label="إغلاق">
                    <X className="h-5 w-5" />
                  </button>
                )}
              </div>
