            ...
            <li key={it.href}>
              <Link
                ...
                className={`relative flex flex-col items-center gap-0.5 rounded-lg px-3 py-1.5 text-[11px] transition ${
                  active ? "font-semibold text-gold-400" : "text-white/70 hover:text-white"
                }`}
              >
                <Icon className={`h-5 w-5 ${active ? "text-gold-400" : "text-white/70"}`} />
                {it.label}
                {it.cart && count > 0 && (
                  <span className="absolute end-1 top-0.5 grid h-4 min-w-4 place-items-center rounded-full bg-gold-500 px-1 text-[10px] font-bold text-brand-900 nums">
                    {count}
                  </span>
                )}
              </Link>
