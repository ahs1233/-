  ...
  if (lines.length === 0) {
    return (
      <div className="py-16 text-center">
        <p className="text-neutral-400">سلتك فارغة</p>
        <Link href="/" className="mt-3 inline-block text-gold-400">
          تصفّح المنتجات
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-4 pb-40 md:pb-24">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-extrabold text-neutral-100">السلة</h1>
        <span className="text-sm text-neutral-400 nums">{lines.length} منتج</span>
      </div>

      <ul className="space-y-3">
        {lines.map((l) => (
          <li key={l.variantId} className="bg-card flex gap-3 rounded-2xl border border-line p-4">
              <Link href={`/product/${l.slug}`} className="bg-card2 h-20 w-20 flex-shrink-0 overflow-hidden rounded">
              ...
              <div className="flex-1">
                <Link href={`/product/${l.slug}`} className="line-clamp-1 font-medium text-neutral-100">
                  {l.title}
                </Link>
                ...
                <div className="mt-1 flex items-center justify-between">
                  ...
                  <span className="font-extrabold text-gold-300 nums">{formatIQD(l.unitPrice * l.quantity)}</span>
                </div>
              </div>
              <button
      ...
      <div className="bg-card/95 fixed inset-x-0 bottom-[calc(3.5rem+env(safe-area-inset-bottom))] z-30 border-t border-line p-3 backdrop-blur-md md:bottom-0">
        <div className="container-app">
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="shrink-0">
              ...
              <div className="text-lg font-extrabold text-gold-300 nums">{formatIQD(subtotal)}</div>
            </div>
